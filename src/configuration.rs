...
pub async fn setup() -> Result<(LocalConfig, CoreConfig)> {
	...
	let file_contents = fs::read_to_string(path).await.context("Error reading configuration file")?;
	let settings: SettingsFile = ron::de::from_str(&file_contents).context("Error parsing configuration file")?;
